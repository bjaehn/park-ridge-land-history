-- Add SECURITY DEFINER so RPCs bypass RLS when called by the anon role.
-- These functions only return aggregated/summarized data so bypassing RLS is safe.

CREATE OR REPLACE FUNCTION city_market_history()
RETURNS TABLE(sale_year integer, sale_count integer, median_price integer)
...
RETURNS TABLE(assessment_year integer, avg_total integer)
LANGUAGE sql STABLE SECURITY DEFINER AS $$
  SELECT
    a.assessment_year,
...
RETURNS TABLE(appeal_year integer, appeal_count integer)
LANGUAGE sql STABLE SECURITY DEFINER AS $$
  SELECT
    a.appeal_year,
    COUNT(*)::integer
...
RETURNS TABLE(permit_year integer, residential_count integer, commercial_count integer)
LANGUAGE sql STABLE SECURITY DEFINER AS $$
  SELECT
...
RETURNS TABLE(decade integer, plat_count integer)
LANGUAGE sql STABLE SECURITY DEFINER AS $$
  SELECT
    (FLOOR(EXTRACT(YEAR FROM s.recorded_date) / 10) * 10)::integer,
    COUNT(*)::integer
  FROM subdivisions s
...
LANGUAGE sql STABLE SECURITY DEFINER AS $$
  SELECT
    s.name,
    EXTRACT(YEAR FROM s.recorded_date)::integer AS recorded_year,
    MIN(p.year_built)::integer AS earliest_built,
    (MIN(p.year_built) - EXTRACT(YEAR FROM s.recorded_date))::integer AS gap_years,
  ...
  WHERE s.recorded_date IS NOT NULL
    ...
    AND p.year_built > EXTRACT(YEAR FROM s.recorded_date)
    AND p.year_built < 2010
  GROUP BY s.id, s.name, s.recorded_date
  HAVING COUNT(psl.pin) >= 3
    AND (MIN(p.year_built) - EXTRACT(YEAR FROM s.recorded_date)) > 0
